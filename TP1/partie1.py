...
def main():
    ...
    # A. Pr(W=1)
    A = (watson * pluie)[:, :, 1, :].sum()
    print('A. Pr(W=1) = {}'.format(A))

    # B. Pr(W=1|H=1)
    B = (pluie * arroseur * holmes * watson)[:, :, 1, 1].sum()/(holmes * pluie * arroseur)[:, :, :, 1].sum()
    print('B. Pr(W=1|H=1) = {}'.format(B))

    # C. Pr(W=1|H=1,A=0)
    C = (pluie * arroseur * holmes * watson)[:, 0, 1, 1].sum()/(holmes * pluie * arroseur)[0, :, :, 1].sum()
    print('C. Pr(W=1|H=1,A=0) = {}'.format(C))

    # D. Pr(W=1|A=0)
    D = (pluie * arroseur * holmes * watson)[:, 0, :, 1].sum()/(arroseur)[:, 0, :, :].sum()
    print('C. Pr(W=1|A=0) = {}'.format(D))

    # E. Pr(W=1|P=1)
    E = (pluie * arroseur * holmes * watson)[1, :, :, 1].sum()/(pluie)[1, :, :, :].sum()
